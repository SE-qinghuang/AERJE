...
def train_model(model, train_loader,
    ...
    for i in range(epoch):
        ...
        torch.save(model.state_dict(), "./saved_model/best_bert_model.pth")

if __name__ =='__main__':

    bert_path = "../base_models/bert_model/"  # 该文件夹下存放三个文件（'vocab.txt', 'pytorch_model.bin', 'config.json'）
    ...
    input_ids_train,input_types_train,input_masks_train,y_train = get_data('../data/classifer_data/train4class.csv')
    input_ids_test,input_types_test,input_masks_test,y_test = get_data('../data/classifer_data/test4class.csv')
    ...
    # 加载最优权重对测试集测试
    model.load_state_dict(torch.load("./saved_model/best_bert_model.pth"))
    pred_test = predict(model, test_loader, DEVICE)
    print("\n Test Accuracy = {} \n".format(accuracy_score(y_test, pred_test)))
    print(classification_report(y_test, pred_test, digits=7))