...
from scripts import eval_extraction as ex
from scripts import sel2record as sel2r
...
from AERJE.extraction import constants
from AERJE.extraction.record_schema import RecordSchema
from AERJE.extraction.predict_parser import decoding_format_dict
from AERJE.extraction.extraction_metrics import get_extract_metrics
from AERJE.extraction.noiser.spot_asoc_noiser import SpotAsocNoiser
from AERJE.extraction.dataset_processer import PrefixGenerator
from AERJE.seq2seq.constrained_seq2seq import ConstraintSeq2SeqTrainingArguments, ConstraintSeq2SeqTrainer
from AERJE.seq2seq.data_collator import (
    DataCollatorForMetaSeq2Seq,
    DynamicSSIGenerator,
)
from AERJE.seq2seq.features import RecordFeature
from AERJE.seq2seq.t5_bert_tokenizer import T5BertTokenizer
...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default='./base_models/AERJE-extractor-en',
...
class DataTrainingArguments:
    ...
    train_file: Optional[str] = field(
        default="./data/text2spotasoc/absa/14lap/train.json", metadata={"help": "The input training data file (a jsonlines or csv file)."}
    )
    validation_file: Optional[str] = field(
        default='./data/text2spotasoc/absa/14lap/val.json',
        metadata={
    ...
    test_file: Optional[str] = field(
        default='./data/text2spotasoc/absa/14lap/test.json',
        metadata={
    ...
    record_schema: str = field(
        default='./data/text2spotasoc/absa/14lap/record.schema', metadata={"help": "The input event schema file."}
    )
    spot_noise: float = field(
        default=0.1, metadata={"help": "The noise rate of null spot."}
    )
    asoc_noise: float = field(
        default=0.1, metadata={"help": "The noise rate of null asoc."}
    )
    meta_positive_rate: float = field(
        default=1.0, metadata={"help": "The keep rate of positive spot."}
    )
...
def main(data_file,outfile):
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,to do all the other way round
    ...
    ##################### data_arg changing #########################
    # model_args.model_name_or_path='./output/cy_RQ1+2/K=UIE'
    source_file_name = data_file
    data_args.train_file = source_file_name + "/train.json"
    # data_args.validation_file = source_file_name + "/val.json"
    data_args.validation_file = None
    data_args.test_file = source_file_name + "/test.json"
    data_args.record_schema = source_file_name + "/record.schema"
    training_args.output_dir = './output/'+ outfile
    ################### train_data args chaning #####################
    ...
    training_args.num_train_epochs = 50.0
    training_args.warmup_ratio = 0.06
    training_args.save_total_limit = 1
    training_args.save_better_checkpoint = True
    training_args.seed = 421
    # training_args.eval_step = 0
    training_args.remove_unused_columns = False
    training_args.load_best_model_at_end = True
    training_args.metric_for_best_model = 'eval_overall-F1'
    ...
    datasets = load_dataset("AERJE_json.py", data_files=data_files)
    ...
    def preprocess_function(examples):
        ...
        ### targets --- <'<extra_id_0> <extra_id_0> aspect <extra_id_5> Screen <extra_id_0> positive <extra_id_5> awesome <extra_id_1> <extra_id_1>>
        inputs = [prefix + inp for inp in inputs]
        ### input = 'Somehow the system clock got messed up after reboot .'
        model_inputs = tokenizer(inputs, max_length=data_args.max_source_length, padding=padding, truncation=True)
        ### 'input_ids': [[8912, 1554, 30, 34, 11, 762, 3, 5, 1]   3,5,1 is end with "."
        ...
        ### <'<extra_id_0> <extra_id_0> aspect <extra_id_5> Screen ... ---> [32099,32099,2663,32094,9937,32099,1465 ...]
        model_inputs['sample_prompt'] = [False] * len(model_inputs['input_ids'])
        ### model_input['sample_prompt'] = False......
        ...
        if data_args.spot_noise > 0 or data_args.asoc_noise > 0:
            if data_args.decoding_format == 'spotasoc':
                spot_asoc_nosier = SpotAsocNoiser(
                    ...
                    null_span=constants.null_span, ### rejection noise ###
                )
            else:
    ...
    # Training
    if training_args.do_train:
        if not os.path.exists(training_args.output_dir + '/train_result'):
            os.makedirs(training_args.output_dir + '/train_result')
            print("mkidr create sucessful")
        ...
        output_train_file = os.path.join(training_args.output_dir + '/train_result', "train_results.txt")
        if trainer.is_world_process_zero():
            ...
            trainer.state.save_to_json(os.path.join(training_args.output_dir + '/train_result', "trainer_state.json"))
    ...
    results = {}
    if training_args.do_eval:
        if not os.path.exists(training_args.output_dir + '/eval_result'):
            os.makedirs(training_args.output_dir + '/eval_result')
            print("mkidr create sucessful")
        ...
        output_eval_file = os.path.join(training_args.output_dir  + '/eval_result', "eval_results_seq2seq.txt")
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                output_test_preds_file = os.path.join(training_args.output_dir  + '/eval_result', "eval_preds_seq2seq.txt")
                with open(output_test_preds_file, "w") as writer:
                    writer.write("\n".join(eval_preds))

    if training_args.do_predict:

        if not os.path.exists(training_args.output_dir + '/test_result'):
            os.makedirs(training_args.output_dir + '/test_result')
            print("mkidr create sucessful")
        ...
        output_test_result_file = os.path.join(training_args.output_dir  + '/test_result', "test_results_seq2seq.txt")
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                output_test_preds_file = os.path.join(training_args.output_dir + '/test_result', "test_preds_seq2seq.txt")
                with open(output_test_preds_file, "w") as writer:
                    writer.write("\n".join(test_preds))
...
if __name__ == "__main__":
    ...
    f_name = './dataset_processing/secondwork/'
    # out  = f_name.split("/")[-1]   #seed1/10-shotyou can refer to this post
    out = 'K=3'
    main(f_name, out)
    sel2r.main(f_name, './output/' + out + '/test_result')
    ex.main(f_name, './output/' + out + '/test_result')
