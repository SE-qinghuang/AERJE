...
class Metric:
    ...
    def count_instance(self, gold_list, pred_list):
        if self.match_mode == 'set':
            ...
            dup_gold_list = deepcopy(gold_list)  ### dup_gold_list = deepcopy(gold_list)###
    ...
    def changed_count_instance_strict(self, gold_list, pred_list):
        if self.match_mode == 'set':
            gold_list = set(gold_list)
            pred_list = set(pred_list)
            if self.verbose:
            ...
            if len(pred_list) != 0 and len(gold_list) !=0:
                for l in range(len(gold_list)):
                    if (gold_list[l][2],gold_list[l][4]) not in offsets_list:
                        offsets_list.append((gold_list[l][2],gold_list[l][4]))
                    if (gold_list[l][4],gold_list[l][2]) not in offsets_list:
                        offsets_list.append((gold_list[l][4],gold_list[l][2]))

                for pred in pred_list:
                    # print(pred)
                    for gold in dup_gold_list:
                        # print(dup_gold_list[i][0],dup_gold_list[i][1],dup_gold_list[i][2],dup_gold_list[i][3],dup_gold_list[i][4])
                        if pred[0] == gold[0] and pred[1] == gold[1] and pred[3] == gold[3]:  ### if pred in dup_gold_list:###
                            if (pred[2],pred[4]) in offsets_list:
                                self.tp += 1
                                num += 1
                                pred_1 = (pred[0], pred[1], pred[2], pred[3], pred[4])
                                pred_2 = (pred[0], pred[1], pred[4], pred[3], pred[2])
                                if self.match_mode == 'normal':
                                    if pred_1 in dup_gold_list:
                                        dup_gold_list.remove(pred_1)
                                    if pred_2 in dup_gold_list:
                                        dup_gold_list.remove(pred_2)
    ...
    def changed_count_instance_boundary(self, gold_list, pred_list):
        if self.match_mode == 'set':
            gold_list = set(gold_list)
            pred_list = set(pred_list)
            if self.verbose:
            ...
            offsets_list = []
            dup_gold_list = deepcopy(gold_list)  ### dup_gold_list = deepcopy(gold_list)###
            if gold_list != [] and dup_gold_list !=[]:
                for l in range(len(gold_list)):
                    if (gold_list[l][1], gold_list[l][2]) not in offsets_list:
                        offsets_list.append((gold_list[l][1], gold_list[l][2]))
                    if (gold_list[l][2], gold_list[l][1]) not in offsets_list:
                        offsets_list.append((gold_list[l][2], gold_list[l][1]))

                for pred in pred_list:
                    for gold in dup_gold_list:
                        if pred[0] == gold[0]:  ### if pred in dup_gold_list:###
                            if (pred[1], pred[2]) in offsets_list:
                                self.tp += 1
                                pred_1 = (pred[0], pred[1], pred[2])
                                pred_2 = (pred[0], pred[2], pred[1])
                                if self.match_mode == 'normal':
                                    if pred_1 in dup_gold_list:
                                        dup_gold_list.remove(pred_1)
                                    if pred_2 in dup_gold_list:
...
class RelationScorer(Scorer):
    ...
    def eval_instance_list(gold_instance_list, pred_instance_list, verbose=False, match_mode='normal'):
        ...
        ### num_list only aimed at the instance that all attribute right. ###
        for pred, gold in zip(pred_instance_list, gold_instance_list):
            pre_string_tp, pre_offset_tp = metrics['string'].tp, metrics['offset'].tp

            # Span Boundary and Type
            num_offset = metrics['offset'].changed_count_instance_strict(
                gold_list=gold.get('offset', []),
                pred_list=pred.get('offset', []),)

            num_string = metrics['string'].changed_count_instance_strict(
                gold_list=gold.get('string', []),
                pred_list=pred.get('string', []), )

            if num_offset == len(gold.get('offset', [])) == len(pred.get('offset', [])) and len(gold.get('offset', []))!=0:
                num_list.append(i)
            i += 1
            ...
            for eval_key in boundary_metrics:
                # Span Boundary Only
                boundary_metrics[eval_key].changed_count_instance_boundary(
