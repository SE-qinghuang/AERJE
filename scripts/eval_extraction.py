...
from AERJE.extraction.scorer import EntityScorer, RelationScorer, EventScorer
...
def main(g,p):
    ...
    parser.add_argument('-g', dest='gold_folder', default= g,help="Golden Dataset folder")  ###data/text2spotasoc/absa/14lap/'
    parser.add_argument('-p', dest='pred_folder', default=[p],nargs='+', help="Predicted model folder")  ###'output/'
    parser.add_argument('-v', dest='verbose', action='store_true', help='Show more information during running')
    parser.add_argument('-w', dest='write_to_file',default=True ,action='store_true', help="Write evaluation results to predicted folder")
        ...
        for data_key, (generation, gold_file) in data_dict.items():
            ...
            for task in task_dict:
                ...
                if task != "relation":
                    ...
                    sub_results,useless = scorer.eval_instance_list(
                        gold_instance_list=gold_instance_list,
                        pred_instance_list=pred_instance_list,
                        verbose=options.verbose,
                        match_mode=options.match_mode,
